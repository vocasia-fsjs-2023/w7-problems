...
function getTotalPrice(productsObj) {
  ...
  for (const productName in productsObj) {
    ...
    const product = listProduct.find((item) => item[0] === productName);
...
function getDiscount(memberStatus, totalPrice) {
  // Write your code here
  const discountRate = memberStatus ? 0.8 : 1.0; // 20% discount for members
...
function shoppingTeros(customer) {
  ...
  const message = customer.member
    ? `Hai pelanggan setia ${customer.name}!`
    : `Hai ${customer.name}!`;
  // EXPECTATION Return berupa string sesuai contoh dibawah
  // apabila member maka panggil dia pelanggan setia
  return `${message} Total Harga yang harus kamu bayar adalah Rp ${discountedPrice}`;
}
