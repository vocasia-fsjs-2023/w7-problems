...
let allMovies = [
  ...
  function mostRecommended(data, category) {
    ...
    for (let i = 0; i < data.length; i++) {
      const movie = data[i];
      if (movie.genre.toLowerCase() === category.toLowerCase()) {//memeriksa film sesuai genre yang dicari
        if (!rateTinggi || movie.rating > rateTinggi.rating) {//jika tidak ada film dengan rating tertinggi atau film ini memiliki rating tertinggi, perbarui rateTinggi
          rateTinggi = movie;
        }
      }
    }
    
    //Kondisi mencocokan rating untuk mengeluarkan message yang sesuai
    if (rateTinggi) {
      output.title = rateTinggi.title;
      output.rating = rateTinggi.rating;
      output.message = `You should watch this ${category} movie directed by ${rateTinggi.director}!`;
    } else {
